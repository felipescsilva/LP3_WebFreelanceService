...
$(function() {
    ...
    let url = `http://localhost:8080/ProjetoLP3/solicitacao?solicitacao-CPF_Anunciante=${user.cpf}`;
    let html = '';
    ...
    .then(function(data) {
        ...
        for(var i = 0; i < data.length ; i++){
            ...
            html += `<p>CPF do Solicitante: ${data[i].CPF_User}</p>`;
            html += '</div>';
            html += '<div>';
            ...
            html += `<p>Valor Total:R$ ${data[i].valorTotal} </p>`;
            html += '</div>';
            html += '<div>';
